...
import { Download, Scissors, FileText, Music, Tv, Radio, LayoutGrid } from "lucide-react";
...
const navItems = [
  ...
  { href: "/transcribe", label: "تفريغ النص", icon: FileText },
  { href: "/to-mp3", label: "تحويل لـ MP3", icon: Music },
  { href: "/anime", label: "التعرف على الأنمي", icon: Tv },
  { href: "/podcast", label: "التعرف على البودكاست", icon: Radio },
];
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-[100dvh] bg-background text-foreground selection:bg-primary/30">
      {/* Sidebar */}
      <aside className="w-72 border-l border-border/50 bg-card/30 flex-shrink-0 flex flex-col hidden md:flex">
        <div className="p-8">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={cn(
                "flex items-center gap-3 px-4 py-3.5 rounded-xl cursor-pointer transition-all font-bold text-base",
